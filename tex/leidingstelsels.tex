...
\begin{voorbeeld}
Een leiding met cirkelvormige doorsnede en lengte 10\unit{m}, diameter 0.10\unit{m} en ruwheid 0.1\unit{mm} verbind twee waterreservoirs. De niveau's van deze reservoirs worden op constante hoogte gehouden en het hoogteverschil tussen de twee reservoirs is 1\unit{m}. De in- en uitlaat van de leiding in de reservoirs is zonder afrondingen uitgevoerd.

Bepaal het debiet door de leiding rekeninghoudend met de ladingsverliezen aan in- en uitlaat.

We kunnen de mechanische energie vergelijking opschrijven tussen twee punten met punt $1$ aan het oppervlak van het hoger gelegen reservor en punt $2$ aan het oppervlak van het lager gelegen reservoir. 
\begin{equation}
	z_1 + \frac{p_1}{\rho g} + \frac{v_1^2}{2 g} - h_\mathrm{L} - h_\mathrm{L,inlaat} - h_\mathrm{L,uitlaat} =  z_2 + \frac{p_2}{\rho g} + \frac{v_2^2}{2 g} \nonumber
\end{equation}

In beide punten is de snelheid bij benadering gelijk aan nul, en de druk atmosfeerdruk. Bovernstaande vergelijking vereenvoudigt zich dus tot:
\begin{equation}
	z_1 -z_2 = h_\mathrm{L} + h_\mathrm{L,inlaat} + h_\mathrm{L,uitlaat}
	\label{eqn:toepassing lokale verliezen mechanische energie}
\end{equation}

De verschillende ladingsverliezen kunnen verder uitgeschrenen worden als:
\begin{align}
...
De weerstandscoefficienten $K_\mathrm{inlaat}$ en $K_\mathrm{uitlaat}$ kunnen we bekomen uit \label{sec:Tabellen en grafieken}. Voor de niet afgeronde in- en uitlaat wordt dit:
\begin{align}
	K_\mathrm{inlaat} = 0.5 \nonumber \\
	K_\mathrm{uitlaat} = 1.0 \nonumber
\end{align}
...
\begin{enumerate}
	\item schat $f$
	\item los (\ref{eqn:toepassing lokale verliezen mechanische energie}) op naar $\dot{V}$
...
\begin{equation}
	\frac{p_2}{\rho g} + \frac{v_2^2}{2 g}  + z_2 = \frac{p_1}{\rho g} + \frac{v_1^2}{2 g}  + z_1 - h_L + h_P
...
\begin{align}
	h_P(\dot{V}) \\
	\frac{p_2-p_1}{\rho g} + \frac{v_2^2-v_1^2}{2 g}  + z_2-z_1 + h_L(\dot{V})
\end{align}
